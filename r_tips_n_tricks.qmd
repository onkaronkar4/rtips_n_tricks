...
## Reformat the long comment lines

```{r}
#xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxXXXXxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx 
```

## Use require instead of library if the said package is not installed in the system the require will install it and if the package is alreayd installed it will just work as library function so use 
```{r}
# require(tidyverse) instead of library(tidyverse)
```


## Anonymyze Names
How to anonymise names with the randomNames package
...
How to mask values in columns with sensitive data
...
ccards |> 
  mutate(creditcards_masked = map_chr(
     creditcards, 
    .f = \(el) str_replace(el, "^.{10}", 
                           replacement = strrep("X", 10))
  ) )
```


## Create dummy data with names, phone numbers, emails, numbers, genes, & more
...
ch_generate(c("name", "job", "phone_number", "currency"), n = 20)
```
...
### current windows user name or the windows home within an R session?

```{r}
username <- Sys.getenv("USERNAME")
print(username)
```
To get the Windows home directory, you can use the USERPROFILE environment variable. Here's an example code:
...
Sys.getenv("HOME")
```
And if you just type, you will see all available environment variables.
```{r}
Sys.getenv()
```

ou can try explicitly setting the environment variable using the Sys.setenv function (note the capital "S" in "Sys"). For example, you can try adding the following line at the beginning of your script:

```{r}
Sys.setenv(R_USER="onkar.a.singh") # this solved the cannot find sys.env function error
```



### setting or editing .Rprofile 

```{r}
usethis::edit_r_profile()

# Add the following line to your .Rprofile file:
Sys.setenv(R_USER="my_user_name"). 
# Replace my_user_name with your actual user name.

# Save the file and exit.

# Now, every time you start a new R session in RStudio, the code in your .Rprofile file will be executed automatically and the R_USER environment variable will be set. This should fix the issue you were having with the sys.setenv() function not being found.
# 
# The usethis package provides many other useful functions for working with R projects, including functions for creating new projects, adding files to projects, and sharing projects on GitHub. You can learn more about the usethis package by typing ?usethis in the R console.


```

### Opening R console | Finding R installation path | R path

If you have R installed on your Windows system and you want to open the R console instead of RStudio, you can follow these steps:

Go to the directory where R is installed. By default, R is installed in "C:\Program Files\R" or "C:\Program Files (x86)\R" depending on whether you have a 64-bit or 32-bit system.

Inside the R installation directory (inside bin folder), find the file "Rgui.exe" or "Rterm.exe" and double-click on it to open the R console.

Alternatively, you can open the R console directly from RStudio by clicking on the "Tools" menu and selecting "Shell" and then "Open System Terminal". This will open a command prompt or terminal window where you can type R to launch the R console.

Note that when you open RStudio, it automatically starts the R console in the background and provides a graphical user interface for working with R. If you prefer to work with the R console directly, you can use one of the methods described above to launch it separately from RStudio.

### finding and fixing the Rpath in the VS Code

To find the R installation path see the descriprtion above R Path. 

its "C:/Program Files/R/R-4.2.2/bin/x64/Rterm.exe" in my laptop

To fix the error you're encountering in VS Code, you need to update the path to the R executable in the VS Code settings. Here's how you can do it:

Open VS Code.

Go to the "Extensions" tab on the left-hand side and search for the "R" extension. Click on the "Install" button to install the extension if it is not already installed.

Once the extension is installed, click on the "Settings" icon (the gear icon) at the bottom left corner of the window.

In the search bar at the top, type "R Path". This will show you the "R: Path" setting.

Click on the "Edit in settings.json" button next to "R: Path". This will open the settings.json file.

In the "R: Path" setting, update the path to the correct location of the R executable file. its "C:/Program Files/R/R-4.2.2/bin/x64/Rterm.exe" in my laptop/window system which worked. Make sure that you include the correct path to the "bin" directory where the R executable is located.
...
### Ctrl+Shift+Alt+M
If you have to change some parts of your code in multiple lines, Ctrl+Shift+Alt+M is a very handy #RStudio shortcut. #RStats
https://twitter.com/RPy_DataScience/status/1504591909922168841
...
The {session_info} function from {sessioninfo} Package creates such a log Memo
...
extract_eq(model,
           ...
           intercept = "beta")
```
group_group_by and slice_max is a powerful combination in Tidyverseby and slice_max is a powerful combination in Tidyversegroup_by and slice_max is a powerful combination in Tidyverse
...
### case_when along with if_any/if_all 

In case you missed it, if_any / if_all can be combined with case_when to create new columns based on a condition across multiple columns.
...
### group_by and slice_max is a powerful combination in Tidyverse 

Here I tried to find the months in the economics dataset with the highest unemployment rate per year.

The trick is that slice_max is applied to each group individually.
...
#### dplyr:correct typos in many columns with mutate across, and str_detect 
...
df <- data.frame(
  id =  1:4, 
  product = c('fruit', 'sauce', 'household', 'pharmacy'), 
  price = c(2.99, 2.1, 4.2, 3.98)
)

# View df
df

# Mutate with case_when Using mutate combined with other functions is very powerful case_when() let's you apply a different value for each case specified

df |> 
  mutate(tax = case_when(
    product == 'fruit' ~ price * 0.03, # for the case when the product is fruit, the tax is price * 3%
    product == 'sauce' ~ price * 0.04, # for the case when the product is sauce, the tax is price * 4%
    product == 'household' ~ price * 0.07, # for the case when the product is household, the tax is price * 7%
    product == 'pharmacy' ~ price * 0.05# for the case when the product is pharmacy, the tax is price * 5%
  ), 
  Total = price * tax)
...
### interval inner_join
[Coming to a dplyr near you (eventually)](https://twitter.com/dvaughan32/status/1474007521157394436)

```{r}
library(fuzzyjoin)
library(dplyr)
x1 <- data.frame(id = 1:3, start = c(1, 5, 10), end = c(3, 7, 15))
x2 <- data.frame(id = 1:3, start = c(2, 4, 16), end = c(4, 8, 20))

inner_join(x1, x2, by = c("start", "end"))
interval_inner_join(x1, x2, by = c("start", "end"))
inner_join(x1, x2, join_by(overalps(x$start, x$end, y$start, y$end)))
```
...
### knitr v1.35 to v1.37: Alternative Syntax for Chunk Options, and Some New Engines

https://yihui.org/en/2022/01/knitr-news/

##### An alternative syntax for chunk options
Chunk options used to be written inside {} in the chunk header, which can be hard to read when there are too many options, or certain options take too long (character) values. For example:
...
knitr::include_graphics("WIN_20210924_164135.JPG", dpi = NA)
```
Since knitr 1.35, we have provided an alternative way to write chunk options. That is, you can write then inside a chunk after the special comments #|, e.g.
```{r}
#| rmdworkflow,
#| echo = FALSE,
...
I love you  
[@rOpenSci](https://www.r-bloggers.com/2019/04/join-split-and-compress-pdf-files-with-pdftools/)

```{r}
# load pdftools
library(pdftools)
#extract some pages
pdf_subset('bigger_file_name.pdf', 
           pages = 3015: 3025, 
...
# breaks. Boo!   
# Quick solution
stringr:: str_squish("This text was taken
from a PDF and there are loads of 
random page
breaks. Boo! ")
# output
...
Ctrl+shift+K opens the command paletter in github with various options. 
...
password: use the personal access token 
...
### [Connecting an existing RStudio project to git and GitHub](https://www.youtube.com/watch?v=bUoN85QvC10)
